...
gas_part_numb = len(x_p)
gas_coords = np.zeros([gas_part_numb, 3], dtype = float_type)
gas_vel = np.zeros([gas_part_numb, 3], dtype = float_type)

gas_masses = np.ones(gas_part_numb, dtype=float_type)
...
part0.create_dataset('ParticleIDs', data=np.arange(0, gas_part_numb))
